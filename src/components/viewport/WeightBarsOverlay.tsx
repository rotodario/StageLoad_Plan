...
function Bar({ x, z, height, color, label, showLabel }: { x: number; z: number; height: number; color: string; label: string; showLabel: boolean }) {
  return (
    <group position={[x, height / 2, z]}>
      ...
      {showLabel && <Html position={[0, height / 2 + 0.18, 0]} center className="axis-label">{label}</Html>}
    </group>
  );
}
